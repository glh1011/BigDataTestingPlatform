...
export default class Aside extends Component {
  ...
  getNavMenuItems = (menusData) => {
    ...
    if (parseInt(localStorage.getItem('userLevel')) == 0){
      return menusData
      .filter((item) => {
        return item.name && !item.hideInMenu && item.zeroVisible
      })
      .map((item, index) => {
        return this.getSubMenuOrItem(item, index);
      });
    }else if(parseInt(localStorage.getItem('userLevel')) == 1){
      return menusData
      .filter((item) => {
        return item.name && !item.hideInMenu && item.firstVisible
      })
      .map((item, index) => {
        return this.getSubMenuOrItem(item, index);
      });
    }else if(parseInt(localStorage.getItem('userLevel')) == 2){
      return menusData
      .filter((item) => {
        return item.name && !item.hideInMenu && item.secondVisible
      })
      .map((item, index) => {
        return this.getSubMenuOrItem(item, index);
      });
